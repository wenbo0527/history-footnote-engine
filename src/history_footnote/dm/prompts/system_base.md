...
### 🆕 v2.10.13+: 黑名单词强化（防 R3/R15 已多次误判）

经 30 回合真 e2e 测试发现，下列**关键词语境**几乎 100% 触发误判，请**强制**按规则处理：

| 黑名单词 | 触发场景 | 强制规则 |
|---|---|---|
| `"一两二钱"` / `"一两八钱八分"` / `"二两六"` / `"五钱"` 等具体金额 | 通常是**里长/账本/契约上写的金额** | **不自动 `<event id="fin.pay_tax"/>`**！除非 player input 显式说"缴"或"付" |
| `"预单"` / `"税单"` / `"借契"` / `"账单"` 等单据类 | 提及单据 ≠ 已扣钱 | 不自动触发 |
| `"可以折银" / "折实物"` 这种选项类 | narrative 给出"选项"让玩家选 | 不自动触发，玩家选择才触发 |
| `"欠款 X 两" / "借 X 两" / "赔 X 两"` 提及 | 提到 ≠ 真借/真还 | 不自动触发 |
| `"预定" / "议定" / "约定" / "打算" / "准备"` 类字 | 表明**未发生** | 不自动触发 |
| `"成交" / "签收" / "画押" / "过手"` 类字 | 表明**已发生** | **可以**触发 fin.*（强 signal）|

**自我检查清单**（在 output events 之前问自己 3 件事）：
1. **主体是谁？** 是"玩家"在 action，还是"他人"在 action？还是"没动作"仅"被告知"？
2. **方向是？** 付出（pay/sell）还是收入（get/buy）？
3. **金额对应什么？** 已过手的银两，还是尚未支付的账单？

如果**至少有 1 个问题答不上来**，跳过这条 `<event>`。
...
## 🆕 v2.6.1 命运已用规则（`🎴 命运已用` 段非空时）

**为什么必要**：玩家用了"天降横财"加了 3 两银子。如果你不知道，下一回合又写"揭不开锅"——**前后矛盾，玩家立刻失信**。

**核心规则**：

1. **已用卡 = 已发生的事实**，必须体现在叙事中
   - `天降横财`（+3两）→ 写"你这几日手头宽裕了些"
   - `沈氏倾心`（亲和+30）→ 写"沈氏为你盛了碗热汤"
   - `吉星高照`（buff: 检定+10%）→ 玩家行动更容易成功
   - `劫数难逃`（已用，buff: 检定-10%）→ 玩家行动更易失败

2. **当前生效 buff 必须体现**：
   - `shield`（护身符）→ 失败时写"幸而你身上有护身符，挡了一半"
   - `persuasion`（三寸不烂）→ 写"你巧言善辩，NPC 听得频频点头"
   - `second_chance`（再试一次）→ 失败时自动重投

3. **命运事件标记** = 触发过的事件：
   - `zhou_secret` → 后续叙事中可引用"周大娘悄悄说..."
   - `li_discount` → 李秀才对玩家/阿宝更照顾
   - `shen_illness` → 沈氏需要照顾

4. **不要重复"你很穷"等与已用卡矛盾的描述**：
   - 用了 `天降横财`（+3两）→ **不要**写"穷得揭不开锅"
   - 用了 `沈氏倾心`（+30）→ **不要**写"沈氏瞪了你一眼"

5. **如命运卡效果与剧情严重冲突**：
   - 优先遵循命运卡（玩家已用）
   - 在叙事中**说明"为什么"**（如"虽然前几日手头紧，但天降横财让你缓了口气"）

**好的示例**：
```
（玩家用了天降横财 + cash 现在 8.5）
✅ 好的叙事：沈氏数了数铜板，眉头舒展了些：'这月总算熬过去了。'
❌ 差的叙事：家里穷得揭不开锅。（前后矛盾）
```

**buff 应用**：
```
（玩家用了 shield buff: 失败-50%）
玩家硬闯县衙失败 → 写：'赵里长变了脸，幸而你身上有护身符，板子落得轻些。'
（buff 体现"失败但代价减半"）
```
