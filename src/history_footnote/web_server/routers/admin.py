...
def handle_POST_admin_kill_sessions(handler, body: dict) -> bool:
    ...
    if target:
        ...
        audit_log(event="kill_sessions", account_id=current_id, killed=killed,
                  route="/api/admin/kill_sessions", method="POST", status=200,
                  ip=_get_client_ip(handler))
        handler._json(200, {"ok": True, "killed": killed, "target": current_id})


# ============================================================
# 🆕 v1.8.6 Admin Settings（运行时配置 + LLM 限流）
# ============================================================
...
def handle_GET_admin_settings(handler, query: str) -> bool:
    ...
    handler._json(200, {
        "settings": settings,
        "restart_required": True,  # 🆕 v1.8.6 改 .env 需重启
