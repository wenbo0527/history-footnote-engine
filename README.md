...
## 🎉 v2.10.32 已发布（剧本模式 + 手动输入支持）

最新版本（2026-08-02）完成了**剧本模式 (Story Mode) 完整实施**：

### 🆕 v2.10.22-32：故事模式 (Story Mode)
- ✅ **零 LLM 剧本引擎**（v2.10.22）：剧本驱动的分支剧情，0 LLM 调用，确定性强
- ✅ **环境描写自动注入**（v2.10.17）：春季·晴·盛泽镇·辰时 等
- ✅ **D&D 检定**（v2.10.22）：skill_check / d20 检定，决定路径
- ✅ **跨章回响**（v2.10.19）：第一章 flag 决定 ch2 intro 文本
- ✅ **3 章剧本**（v2.10.26-28）：家贫 → 织染 → 丝绢案，77 节点 100% 已迁移
- ✅ **多声部叙事**（v2.10.25）：旁白 + NPC + 内心独白 + 音效 + 动作
- ✅ **NodeFilter**（v2.10.30）：required_city / required_flags / forbidden_flags
- ✅ **AutoNextNode**（v2.10.31）：无选项时自动跳到下一节点
- ✅ **手动输入支持**（v2.10.32）：关键词模糊匹配，玩家可自由输入中文意图

### 故事模式特点
- **77 个剧本节点**：3 章 × 22-33 节点
- **109+ 单元测试**：100% 通过
- **零 LLM 调用**：剧本完全确定，不依赖 AI 状态
- **手动输入**：玩家输入"借钱"自动匹配到 `borrow_money` 选项
- **多声部渲染**：父亲咳嗽、张氏忧愁、内心独白、织机声等
- **架构评分**：5/5 ⭐

详见 [docs/story-mode/architecture-v2.10.32.md](docs/story-mode/architecture-v2.10.32.md)

### 历史里程碑
- ✅ **v2.10.15 稳定性 + 史实校验双升级**：30/30 PASS
- ✅ **v2.10.10 前端切换**：生产部署从 v1.7.27 旧前端 → SvelteKit v2.x 真实前端
- ✅ **v2.10.8 5 处移动端适配**：输入条 iOS 安全 + sidebar 折叠 + popover 宽度
- ✅ **v2.10.1 涌现式章节架构**
- ✅ **v2.10.6 开局剧情带入完整 6 段**
